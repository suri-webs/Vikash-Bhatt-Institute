...
import { FilePlus, CalendarIcon, BookOpen, Link2, Hash, X } from "lucide-react";
...
interface Props {
    ...
    onSave: (data: { subject: string; week: string; url: string; month: string }) => Promise<void>;
}
...
export default function AddResultDialog({ open, onOpenChange, onSave }: Props) {
    ...
    async function handleSave() {
        ...
        await onSave({ subject, week, url, month });
            ...
            <DialogContent className="sm:max-w-lg p-0 gap-0 overflow-hidden rounded-2xl border border-gray-200 shadow-xl">
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex gap-2.5">
                    ...
                    <button
                        type="button"
                        onClick={handleSave}
                        disabled={adding || !subject.trim() || !week.trim() || !url.trim()}
